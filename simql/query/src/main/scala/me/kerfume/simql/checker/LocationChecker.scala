...
class LocationChecker extends QueryChecker {
  def check(ast: Query, env: ASTMetaData): Result[Unit] = {
    LocationCheckVisitor.visit(ast).run(env).map(_ => ())
  }
}
...
object LocationCheckVisitor extends ASTVisitor {
  ...
  override def visitJoin(node: Join): RE[Join] = re { env =>
    for {
      _ <- Either.cond(allowedToTable(node.rhsTable, env.globalScope), (), UnhandleError("illegal location."))
      res <- super.visitJoin(node).run(env)
    } yield res
  }

  override def visitFrom(node: From): RE[From] = re { env =>
    for {
      _ <- Either.cond(allowedToTable(node.lhs, env.globalScope), (), UnhandleError("illegal location."))
      res <- super.visitFrom(node).run(env)
    } yield res
  }

  override def visitSelect(node: Select): RE[Select] = re { env =>
    for {
      _ <- node.columns.toList.mapE { c =>
        Either.cond(allowedToColumn(c, env.globalScope), (), UnhandleError("illegal location."))
      }
      res <- super.visitSelect(node).run(env)
    } yield res
  }

  override def visitOrder(node: Order): RE[Order] = re { env =>
    for {
      _ <- node.columns.toList.mapE { c =>
        Either.cond(allowedToColumn(c, env.globalScope), (), UnhandleError("illegal location."))
      }
      res <- super.visitOrder(node).run(env)
    } yield res
  }
